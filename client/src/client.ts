...
function createConnection(
    ...
    let result: IConnection = {
        ...
        trace: (value: Trace, tracer: Tracer, sendNotification: boolean = false): void =>
        ...
        didChangeConfiguration: (params: DidChangeConfigurationParams) =>
            connection.sendNotification(DidChangeConfigurationNotification.type, params),
        didChangeWatchedFiles: (params: DidChangeWatchedFilesParams) =>
            connection.sendNotification(DidChangeWatchedFilesNotification.type, params),
        ...
        didChangeTextDocument: (params: DidChangeTextDocumentParams) =>
            connection.sendNotification(DidChangeTextDocumentNotification.type, params),
        didCloseTextDocument: (params: DidCloseTextDocumentParams) =>
            connection.sendNotification(DidCloseTextDocumentNotification.type, params),
        ...
        if (this.restarts.length < 5) {
            ...
            if (diff <= 3 * 60 * 1000) {
                Window.showErrorMessage(
                    `The ${this.name} server crashed 5 times in the last 3 minutes. The server will not be restarted.`
                );
...
export interface ProvideCodeActionsSignature {
    (document: TextDocument, range: VRange, context: VCodeActionContext, token: CancellationToken): ProviderResult<
        (VCommand | VCodeAction)[]
    >;
}
...
export interface ProvideDocumentRangeFormattingEditsSignature {
    (document: TextDocument, range: VRange, options: VFormattingOptions, token: CancellationToken): ProviderResult<
        VTextEdit[]
    >;
}
...
export interface ProvideRenameEditsSignature {
    (document: TextDocument, position: VPosition, newName: string, token: CancellationToken): ProviderResult<
        VWorkspaceEdit
    >;
}
                        ...
                        if (this._changeDelayer) {
                            ...
                            this._changeDelayer.delayer.trigger(() => {
                                this._client.sendNotification(
                                    ...
                                    this._client.code2ProtocolConverter.asChangeTextDocumentParams(event.document)
                                );
                            });
                        } else {
                            this._changeDelayer = {
...
class FileSystemWatcherFeature implements DynamicFeature<DidChangeWatchedFilesRegistrationOptions> {
    ...
    public register(_method: RPCMessageType, data: RegistrationData<DidChangeWatchedFilesRegistrationOptions>): void {
        ...
        for (let watcher of data.registerOptions.watchers) {
            ...
            let fileSystemWatcher: VFileSystemWatcher = Workspace.createFileSystemWatcher(
                watcher.globPattern,
                !watchCreate,
                !watchChange,
                !watchDelete
...
abstract class WorkspaceFeature<T> implements DynamicFeature<T> {
    ...
    public register(message: RPCMessageType, data: RegistrationData<T>): void {
        if (message.method !== this.messages.method) {
            throw new Error(
                `Register called on wron feature. Requested ${message.method} but reached feature ${
                    this.messages.method
                }`
            );
        }
    ...
    protected registerLanguageProvider(options: CompletionRegistrationOptions): Disposable {
        ...
        let provideCompletionItems: ProvideCompletionItemsSignature = (document, position, context, token) => {
            return client
                ...
                .then(client.protocol2CodeConverter.asCompletionResult, error => {
        ...
        let resolveCompletionItem: ResolveCompletionItemSignature = (item, token) => {
            return client
                .sendRequest(CompletionResolveRequest.type, client.code2ProtocolConverter.asCompletionItem(item), token)
                .then(client.protocol2CodeConverter.asCompletionItem, error => {
    ...
    protected registerLanguageProvider(options: SignatureHelpRegistrationOptions): Disposable {
        ...
        let providerSignatureHelp: ProvideSignatureHelpSignature = (document, position, token) => {
            return client
                ...
                .then(client.protocol2CodeConverter.asSignatureHelp, error => {
    ...
    protected registerLanguageProvider(options: TextDocumentRegistrationOptions): Disposable {
        ...
        let provideDocumentHighlights: ProvideDocumentHighlightsSignature = (document, position, token) => {
            return client
                ...
                .then(client.protocol2CodeConverter.asDocumentHighlights, error => {
    ...
    protected registerLanguageProvider(options: TextDocumentRegistrationOptions): Disposable {
        ...
        let provideDocumentSymbols: ProvideDocumentSymbolsSignature = (document, token) => {
            return client
                ...
                .then(client.protocol2CodeConverter.asSymbolInformations, error => {
...
class CodeLensFeature extends TextDocumentFeature<CodeLensRegistrationOptions> {
    ...
    protected registerLanguageProvider(options: CodeLensRegistrationOptions): Disposable {
        ...
        let provideCodeLenses: ProvideCodeLensesSignature = (document, token) => {
            return client
                .sendRequest(CodeLensRequest.type, client.code2ProtocolConverter.asCodeLensParams(document), token)
        ...
        let resolveCodeLens: ResolveCodeLensSignature = (codeLens, token) => {
            return client
                .sendRequest(CodeLensResolveRequest.type, client.code2ProtocolConverter.asCodeLens(codeLens), token)
...
class DocumentOnTypeFormattingFeature extends TextDocumentFeature<DocumentOnTypeFormattingRegistrationOptions> {
    ...
    protected registerLanguageProvider(options: DocumentOnTypeFormattingRegistrationOptions): Disposable {
        ...
        let provideOnTypeFormattingEdits: ProvideOnTypeFormattingEditsSignature = (
            document,
            position,
            ch,
            options,
    ...
    protected registerLanguageProvider(options: TextDocumentRegistrationOptions): Disposable {
        ...
        let provideRenameEdits: ProvideRenameEditsSignature = (document, position, newName, token) => {
            ...
            return client
                ...
                .then(client.protocol2CodeConverter.asWorkspaceEdit, (error: ResponseError<void>) => {
    ...
    protected registerLanguageProvider(options: DocumentLinkRegistrationOptions): Disposable {
        ...
        let provideDocumentLinks: ProvideDocumentLinksSignature = (document, token) => {
            return client
                ...
                .then(client.protocol2CodeConverter.asDocumentLinks, (error: ResponseError<void>) => {
        ...
        let resolveDocumentLink: ResolveDocumentLinkSignature = (link, token) => {
            return client
                .sendRequest(DocumentLinkResolveRequest.type, client.code2ProtocolConverter.asDocumentLink(link), token)
                .then(client.protocol2CodeConverter.asDocumentLink, (error: ResponseError<void>) => {
...
export abstract class BaseLanguageClient {
    ...
    public constructor(id: string, name: string, clientOptions: LanguageClientOptions) {
        ...
        this._c2p = c2p.createConverter(
            clientOptions.uriConverters ? clientOptions.uriConverters.code2Protocol : undefined
        );
    ...
    private handleRegistrationRequest(params: RegistrationParams): Thenable<void> {
        return new Promise<void>((resolve, reject) => {
            for (let registration of params.registrations) {
                ...
                if (!feature) {
                    reject(
                        new Error(`No feature implementation for ${registration.method} found. Registration failed.`)
                    );
                    return;
                }
    ...
    private handleApplyWorkspaceEdit(params: ApplyWorkspaceEditParams): Thenable<ApplyWorkspaceEditResponse> {
        ...
        return Workspace.applyEdit(this._p2c.asWorkspaceEdit(params.edit)).then(value => {
