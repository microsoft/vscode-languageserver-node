...
// protocol/src/browser/test -> protocol
...
// web APIs only. The single Node touch point is the tests' `import 'assert'`, which
// resolves to the `assert` npm package; that package reads `process.env.NODE_DEBUG`
// at load, so we define it away. No Node polyfill plugin is required.
