...
import { SyntaxKind, ClassDeclaration, Node } from '@typescript/native/unstable/ast';
...
export class Symbols {
	...
	constructor(_project: ts.Project, typeChecker: ts.Checker) {
	...
	public createKey(symbol: ts.Symbol): string {
		...
		if (fragments.length > 1) {
			fragments.sort((a, b) => {
				const result = a.p < b.p ? -1 : (a.p > b.p ? 1 : 0);
				if (result !== 0) {
					return result;
				}
				return a.i - b.i;
